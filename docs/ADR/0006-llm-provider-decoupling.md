...
1. **Abstracción vía Interfaces:** El dominio solo interactuará con la IA a través de las firmas definidas en `src/domain/interfaces.py` (como `AIService`). 
2. **Aislamiento en Infraestructura:** El código específico de SDKs, formateo de payloads, manejo de reintentos y lógica de clientes de Ollama, OpenAI o cualquier otro proveedor residirá única y exclusivamente en la capa de infraestructura (`src/infrastructure`).
